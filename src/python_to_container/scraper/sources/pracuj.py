...
def _offers_query(data: dict) -> dict | None:
...
def fetch(query: str, max_pages: int = 3) -> list[Offer]:
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            payload = _offers_query(data) if data else None
        ...
        for g in groups:
            ...
            offers.append(Offer(
                ...
                text=(g.get("jobDescription") or g.get("aboutProjectShortDescription") or "")[:400],
            ))
        polite()
        if len(groups) < 50:
            break
