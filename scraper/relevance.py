...
BUCKETS: dict[str, re.Pattern] = {
    ...
    "ai_core": re.compile(
        ...
        r"uczenia maszynowego|sztucznej inteligencji", re.I),
    ...
    "data_platform": re.compile(
        ...
        r"data platform|inżynier danych|devops", re.I),
    # Where the user is now.
    "qa": re.compile(
        r"\b(qa|sdet|test|tester|testing)\b|automation engineer|"
        r"automatyk|testów|testera", re.I),
}
...
NOISE = re.compile(
    ...
    r"lawyer|prawnik|handlow|sprzedaż|manager of|scrum master)\b", re.I)


def bucket(offer) -> str | None:
...
def split(offers: list) -> dict[str, list]:
    out: dict[str, list] = {"ai_core": [], "data_platform": [], "qa": []}
    for o in offers:
        b = bucket(o)
        if b:
            out[b].append(o)
